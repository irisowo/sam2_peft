...
class PromptGenerator(nn.Module):
    ...
    def _prep_image_prompt(self, x_img: torch.Tensor) -> torch.Tensor:
        ...
        if self.input_type == "fft":
            ...
            return (
                self.prompt.unsqueeze(0)
                .to(x_img.device)
                .repeat(x_img.shape[0], 1, 1, 1)
            )
        else:
            raise ValueError(
    ...
    @torch.no_grad()
    def _fft_prompt(
        self, x: torch.Tensor, rate: float, prompt_type: str
    ) -> torch.Tensor:
        device = x.device
    ...
    def init_prompt(
        ...
        handcrafted_feature: torch.Tensor,
        stage_idx: int,
    ):
    ...
    def add_prompt(self, x: torch.Tensor, prompt, stage_idx: int, depth_idx: int):
