...
class DoRALinear(nn.Module):
    ...
    def __init__(self, linear_layer: nn.Linear, r, alpha, dropout):
        ...
        self.dora_m = nn.Parameter(linear_layer.weight.norm(p=2, dim=0, keepdim=True))
...
class EDoRALinear(nn.Module):
    ...
    def __init__(self, linear_layer: nn.Linear, r: int, init_std: float = 0.01):
        super().__init__()
        self.linear = linear_layer
        self.r = r
        ...
        self.register_buffer("B", (U_r * S_r.unsqueeze(0)).contiguous())  # (m, r)
...
class RLRRLinear(nn.Module):
    ...
    def __init__(self, linear_layer: nn.Linear):
        ...
        ), "RLRR shift bias requires base linear to have bias."
        self.rlrr_shift_bias = (
            nn.Parameter(torch.empty(1, self.out_features))
    ...
    def forward(self, x):
        ...
        if self.linear.bias is not None:
            bias = self.linear.bias + (
                self.rlrr_shift_bias if self.rlrr_shift_bias is not None else 0
            )
        else:
            bias = None
