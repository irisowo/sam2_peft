        ...
        if multi_instance:
            ...
            for comp_id in range(1, num_labels):  # 0 is background
                ...
                if comp_mask.sum() < min_area:
                    continue
                self.components[comp_id] = torch.from_numpy(comp_mask.astype(np.uint8))

            # obj_id is consecutive and start from 1
            # map {comp_id_original -> new_id_starting_from_1}
            remap = {}
            for new_id, old_id in enumerate(sorted(self.components.keys()), start=1):
                remap[old_id] = new_id
            self.components = {
                remap[old_id]: mask for old_id, mask in self.components.items()
            }
        else:
