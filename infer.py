...
class InferModel:
    ...
    def __init__(self, model_cfg, model_ckpt, save_dir, infer_cdf="03"):
        ...
        # Init Paths
        self.img_folder = Path(f"data/infer_{infer_cdf}/test_images")
    ...
    def _get_save_path(self, img_path: Path, is_anomaly_mask=False):
        if is_anomaly_mask:
            save_mask_path = Path(
                self.save_dir / "anomaly_mask" / f"{img_path.stem}_anomaly_mask.png"
            )
    ...
    def _get_grouped_imgpaths(self):
        ...
        print(f"Found {len(img_list)} images in {self.img_folder}")
    ...
    def _infer_img(self, img_path: Path):
        ...
        input_points, input_labels, input_boxes = get_inputs_from_mask(mask_path)
        ...
        for input_pt, input_lb, input_b in zip(input_points, input_labels, input_boxes):
    ...
    def _infer_patch(
        ...
        full_mask = np.zeros((H, W), dtype=np.float16)

        # ---- helper: predict mask for given boxes ----
        def _predict_mask_for_boxes(rgb_img: np.ndarray, boxes: list, desc: str = ""):
            h, w = rgb_img.shape[:2]
            feats = None
            # if len(boxes) == 0:
            ...
            try:
                ...
                print(
                    f"Predict failed for {desc or img_path}, "
        ...
        def _build_patch_regions(H: int, W: int, patch_size: int, overlap: int):
        ...
        patch_boxes = (
            _assign_boxes_to_patches(input_boxes, patch_regions) if use_box else []
        )
        ...
        for idx, (x1, y1, x2, y2) in enumerate(patch_regions):
            ...
            full_mask[y1:y2, x1:x2] = np.maximum(full_mask[y1:y2, x1:x2], patch_mask)
            patch_scores.append(patch_score)

        # --- Normalize ---
        full_mask = (full_mask > 0).astype(np.uint8) * 255
        patch_score = np.array(patch_scores).mean() if len(patch_scores) > 0 else 0.0
    ...
    def infer_imgs(
        ...
        cid_imgpaths = dict(list(cid_imgpaths.items()))  # debug with 10 CIDs
        ...
        for i, (cid, img_paths) in enumerate(tqdm.tqdm(cid_imgpaths.items())):
            ...
            for img_path in train_img_list:
                best_mask = self._infer_patch(
                    img_path=img_path, use_box=use_box, patch_size=cur_patch_size
                )
                cid_all_component_mask = np.maximum(cid_all_component_mask, best_mask)
            ...
            if ad_task:
                ...
                if LOG_CID:
                    print(f"[Anomaly Detection] CID {i+1}/{len(cid_imgpaths)}: {cid}")
                train_embeds = []
                for img_path in train_img_list:
                    image = Image.open(img_path)
                    image = np.array(image.convert("RGB"))
                    full_feats = stitch_sam2_patch_feats(
                        self.predictor, full_image=image, patch_size=1024
                    )
                ...
                for img_path in test_img_list:
                    ...
                    # Save paths
                    anomaly_bin_save_path = self._get_save_path(
                        img_path, is_anomaly_mask=True
                    )
                    anomaly_rawfeat_save_path = anomaly_bin_save_path.with_suffix(
                        ".rawfeat.png"
                    )
                    anomaly_feat_save_path = anomaly_bin_save_path.with_suffix(
                        ".feat.png"
                    )
                    anomaly_rawbin_save_path = str(anomaly_bin_save_path).replace(
                        "_anomaly_mask.png", "_anomaly_rawmask.png"
                    )
                    anomaly_vis_save_path = anomaly_bin_save_path.with_suffix(".jpg")

                    # Get anomaly map
                    anomaly_map = bank.cal_anomaly_map(full_feats)
                    anomaly_map = cv2.resize(
                        anomaly_map, (image.shape[1], image.shape[0])
                    )

                    # Raw anomaly map
                    anomaly_rawmap_gray = (
                        (anomaly_map * 255).clip(0, 255).astype(np.uint8)
                    )
                    cv2.imwrite(anomaly_rawfeat_save_path, anomaly_rawmap_gray)

                    # Raw anomaly map + Instance Masking
                    anomaly_map_gray = anomaly_rawmap_gray * (
                        cid_all_component_mask > 0
                    )
                    # anomaly_map_gray = (anomaly_map_gray * 255).clip(0, 255).astype(np.uint8)
                    cv2.imwrite(str(anomaly_feat_save_path), anomaly_map_gray)

                    # Raw anomaly map binarization
                    anomaly_rawbin_map = postprocess_anomaly_mask(
                        anomaly_rawmap_gray, thresh="mix"
                    )
                    cv2.imwrite(anomaly_rawbin_save_path, anomaly_rawbin_map)

                    # Anomaly map binarization + Instance Masking
                    anomaly_bin_map = anomaly_rawbin_map * (cid_all_component_mask > 0)
                    cv2.imwrite(str(anomaly_bin_save_path), anomaly_bin_map)

                    # Overlap Visualization
                    overlap_anomaly = get_overlap_img(img_path, anomaly_bin_map)
                    cv2.imwrite(str(anomaly_vis_save_path), overlap_anomaly)

                    # Aggressively delete giant map variables internally to drop Peak RAM bounds
                    del (
                        full_feats,
                        anomaly_map,
                        anomaly_rawmap_gray,
...
class InferCDFModel(InferModel):
    ...
    def __init__(
        ...
        self.img_folder = infer_img_dir
        self.gt_folder = Path(
            self.img_folder.parent / self.img_folder.name.replace("images", "masks")
        )
...
def infer(
    model_type="DORA_r16(0207051004)_stage2",
    epoch="100",
    box_type="(gtbox)",
    infer_img_dir="data/infer_08",
    save_dir="data/infer_08/DORA_r16(0207051004)_stage2(Epo100)(nobox)",
):

    init_env()
    assert infer_img_dir is not None, "infer_img_dir must be provided"

    # Config
    base = BaseConfig()
    model_cfg = base.get_sam_cfg(model_type)
    sam2_checkpoint = base.get_sam_ckpt(model_type, epoch=epoch)
    save_dir = Path(save_dir)

    # Init
    infercls = InferCDFModel(
        model_cfg,
        sam2_checkpoint,
        save_dir,
        # additional args for InferCDF
        infer_img_dir=Path(infer_img_dir),
    )

    # Infer
    use_box = False if "nobox" in box_type else True
    infercls.infer_imgs(use_box=use_box)


def demo():
    img_path = "data/infer.jpg"
    model_cfg = "configs/sam2.1/sam2.1_hiera_l_DORA_r16.yaml"
    model_ckpt = "sam2_logs/DORA_r16(0207051004)_stage2/checkpoints/checkpoint_100.pt"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="where to save results, default to data/infer_{infer_cdf}/{model_type}(Epo{epoch}){box_type}",
    )
    parser.add_argument("--model_type", type=str, default="DORA_r16(0207051004)_stage2")
    ...
    infer_img_dir = args.infer_img_dir
    save_dir = args.save_dir
    if args.infer_img_dir is None:
        infer_img_dir = f"data/infer_{cdf}/test_images"
    if args.save_dir is None:
        save_dir = f"data/infer_{cdf}/{model_type}(Epo{epoch}){box_type}"

    model_save_path = Path(f"sam2_logs/{model_type}/checkpoints/checkpoint_{epoch}.pt")

    # Download checkpoints
    download_cmd = f'python sam2_logs/download_ckpt.py download --model "{model_type}" --epoch {epoch} --save_dir sam2_logs'
    if not model_save_path.exists():
        print(
            f"⚠️ Model checkpoint not found at {model_save_path}. Downloading checkpoint with command:\n\t{download_cmd}"
        )
        try:
        ...
        print(
            f"Model checkpoint already exists at {model_save_path}, skip downloading."
        )
