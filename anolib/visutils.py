...
def get_overlap_img(test_img_path: str, mask_img_path: str) -> np.ndarray:
    ...
    mask = cv2.resize(
        mask, (test_img.shape[1], test_img.shape[0]), interpolation=cv2.INTER_LINEAR
    )
...
def show_mask(mask, ax, random_color=False, borders=True):
    ...
    if borders:
        ...
        mask_image = cv2.drawContours(
            mask_image, contours, -1, (1, 1, 1, 0.5), thickness=1
        )
...
def show_box(box, ax):
    ...
    ax.add_patch(
        plt.Rectangle(
            (x0, y0), w, h, edgecolor="yellowgreen", facecolor=(0, 0, 0, 0), lw=2
        )
    )
...
def show_masks(
    ...
    if mode == "horizontal":
        ...
        for box in box_coords:
            ...
            overlap_img = cv2.rectangle(
                overlap_img, (x0, y0), (x0 + w_box, y0 + h_box), (0, 255, 0), 2
            )
